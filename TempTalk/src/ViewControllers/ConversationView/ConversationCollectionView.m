...
//    OWSLogDebug(@"------ contentOffset:%@", NSStringFromCGPoint(contentOffset));
}

- (void)setContentInset:(UIEdgeInsets)contentInset {
    UIEdgeInsets oldContentInset = self.contentInset;
    BOOL isChanged = !UIEdgeInsetsEqualToEdgeInsets(oldContentInset, contentInset);
    
    [super setContentInset:contentInset];
    
    if (isChanged) {
        [self.layoutDelegate collectionViewDidChangeContentInset:oldContentInset to:contentInset];
    }
}
