...
class InputAccessoryViewPlaceholder: UIView {
    ...
    @objc
    public func setKeyboardDismiss() {
        keyboardState = .dismissed
    }
    
    @objc
    public func syncKeyboardState() {
        // Force sync keyboard state when returning from background
        // This helps handle cases where keyboard notifications might be missed
        let currentKeyboardFrame = superview?.frame
        let appFrame = CurrentAppContext().frame
        
        if let keyboardFrame = currentKeyboardFrame,
           keyboardFrame.maxY < appFrame.maxY {
            // Keyboard is visible, ensure we're in the correct state
            if case .dismissed = keyboardState {
                keyboardState = .presented
            }
        } else {
            // Keyboard is not visible, ensure we're dismissed
            if case .presented = keyboardState {
                keyboardState = .dismissed
            }
        }
    }
