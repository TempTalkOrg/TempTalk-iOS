...
- (CGSize)measureSizeWithMaxMessageWidth:(CGFloat)maxMessageWidth minMessageWidth:(CGFloat)minMessageWidth
{
...
- (void)createContentsWithConversationStyle:(ConversationStyle *)conversationStyle
{
    ...
    __block NSString *fileExtension = filename.pathExtension;
    if (fileExtension.length < 1) {
        fileExtension = [MIMETypeUtil fileExtensionForMIMEType:self.attachmentStream.contentType];
    }
    
    [[MIMETypeUtil fileIconToMIMETypesMap] enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, NSArray *obj, BOOL * _Nonnull stop) {
        if([obj containsObject:fileExtension]){
            fileExtension = key;
        }
    }];
    
    
    UIImage *attachmentTypeImage = [UIImage imageNamed:[NSString stringWithFormat:@"attachment_%@",fileExtension.lowercaseString]];
    if(attachmentTypeImage){
        imageView.image = attachmentTypeImage;
    }else{
    ...
    if (topText.length < 1) {
        topText = Localized(@"GENERIC_ATTACHMENT_LABEL", @"A label for generic attachments.");
    }
