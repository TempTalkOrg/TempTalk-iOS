//
//  DTTextSelectionHighlightingView.swift
//  Difft
//
...
class DTTextSelectionHighlightingView: UIView {
    ...
    var useModernPathCalculation: Bool = false
    ...
    func updateImage() {
        ...
        let (offset, image) = generateRectsImage(
            color: self.color,
            rects: self.rects,
            inset: self.inset,
            outerRadius: self.outerRadius,
            innerRadius: self.innerRadius,
            stroke: self.borderOnly,
            strokeWidth: self.strokeWidth,
            useModernPathCalculation: self.useModernPathCalculation
        )

        if let image = image {
            imageView.image = image
            imageView.frame = CGRect(origin: offset, size: image.size)
        }
    }
}
