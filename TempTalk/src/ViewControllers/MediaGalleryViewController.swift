...
public struct GalleryDate: Hashable, Comparable, Equatable {
    ...
    var localizedString: String {
        if isThisMonth {
            return Localized("MEDIA_GALLERY_THIS_MONTH_HEADER", comment: "Section header in media gallery collection view")
        } else if isThisYear {
...
class MediaGalleryViewController: OWSNavigationController, MediaGalleryDataSource, MediaTileViewControllerDelegate {
    ...
    override func applyTheme() {
        view.backgroundColor = Theme.isDarkThemeEnabled ? .ows_black : .ows_white
    ...
    public func presentDetailView(fromViewController: UIViewController, mediaMessage: TSMessage, replacingView: UIView) {
        ...
        self.databaseStorage.read { readTransaction in
            galleryItem = self.buildGalleryItem(message: mediaMessage, transaction: readTransaction)
        }
...
//        UIApplication.shared.isStatusBarHidden = false
        ...
        if isAnimated {
            UIView.animate(withDuration: changedItems ? 0.25 : 0.18,
                ...
                detailView.backgroundColor = Theme.isDarkThemeEnabled ? .ows_black : .ows_white
            })
    ...
    func buildGalleryItem(message: TSMessage, transaction: SDSAnyReadTransaction) -> MediaGalleryItem? {
        guard let attachmentStream = message.attachment(with: transaction) as? TSAttachmentStream else {
//            owsFailDebug("\(self.logTag) in \(#function) attachment was unexpectedly empty")
            return nil
        }
