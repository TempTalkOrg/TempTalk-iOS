...
class MediaPageViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, MediaDetailViewControllerDelegate, MediaGalleryDataSourceDelegate {
    ...
    private func setCurrentItem(_ item: MediaGalleryItem, direction: UIPageViewController.NavigationDirection, animated isAnimated: Bool) {
    ...
    init(initialItem: MediaGalleryItem, mediaGalleryDataSource: MediaGalleryDataSource, options: MediaGalleryOption) {
        ...
        super.init(transitionStyle: .scroll,
                   ...
                   options: [UIPageViewController.OptionsKey.interPageSpacing: kSpacingBetweenItems])
...
//            self.view.backgroundColor = shouldHideToolbars ? UIColor.black : UIColor.white
    ...
    public func didPressDelete(_ sender: Any) {
        ...
        let deleteAction = UIAlertAction(title: Localized("TXT_DELETE_TITLE", comment: ""),
    ...
    private func buildGalleryPage(galleryItem: MediaGalleryItem) -> MediaDetailViewController? {
        ...
        databaseStorage.uiRead { transaction in
            ...
            guard let thread = thread else {
                owsFailDebug("can't fetch thread from message: \(message)")
                return
            }
            ...
            fetchedItem = ConversationInteractionViewItem(interaction: message,
                                                          thread: thread,
                                                          conversationViewMode: ConversationViewMode.unKnow,
...
//                                               conversationStyle: conversationStyle)
    ...
    public func mediaDetailViewController(_ mediaDetailViewController: MediaDetailViewController, requestDelete conversationViewItem: ConversationViewItem) {
        ...
        guard let message = conversationViewItem.interaction as? TSMessage else {
            owsFailDebug("\(logTag) in \(#function) unexpected interaction: \(conversationViewItem)")
            self.presentingViewController?.dismiss(animated: true)
    ...
    private func senderName(message: TSMessage) -> String {
        ...
        case is TSOutgoingMessage:
            return Localized("MEDIA_GALLERY_SENDER_NAME_YOU", comment: "Short sender label for media sent by you")
        default:
            owsFailDebug("\(logTag) Unknown message type: \(message)")
            return ""
        }
    }
