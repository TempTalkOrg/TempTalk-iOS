...
public class SafetyNumberConfirmationAlert: NSObject {
    ...
    public func presentIfNecessary(recipientIds: [String], confirmationText: String, completion: @escaping (Bool) -> Void, beforePresentationHandler: (() -> Void)? = nil) -> Bool {
           ...
           let titleFormat = Localized("CONFIRM_SENDING_TO_CHANGED_IDENTITY_TITLE_FORMAT",
           ...
           let bodyFormat = Localized("CONFIRM_SENDING_TO_CHANGED_IDENTITY_BODY_FORMAT",
           ...
           let confirmAction = UIAlertAction(title: confirmationText, style: .default) { _ in
               ...
               self.databaseStorage.asyncWrite { (transaction) in
                   OWSIdentityManager.shared().setVerificationState(.default, identityKey: untrustedIdentity.identityKey, recipientId: untrustedIdentity.recipientId, isUserInitiatedChange: true, isSendSystemMessage: false, transaction: transaction)
               } completion: {
                   completion(true)
               }
           }
           ...
           let showSafetyNumberAction = UIAlertAction(title: Localized("VERIFY_PRIVACY", comment: "Label for button or row which allows users to verify the safety number of another user."), style: .default) { _ in
    ...
    private func unverifiedIdentitysForSending(recipientIds: [String]) -> [OWSRecipientIdentity]? {
        return recipientIds.compactMap {
            OWSIdentityManager.shared().unverifiedIdentityForSending(toRecipientId: $0)
        }
    }
