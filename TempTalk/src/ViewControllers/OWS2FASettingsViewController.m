...
- (void)viewDidLoad
{
    ...
    self.title = Localized(@"ENABLE_2FA_VIEW_TITLE", @"Title for the 'enable two factor auth PIN' views.");
...
- (void)createPinTextfield
{
    ...
    self.pinTextfield.textColor = Theme.tprimaryColor;
...
- (void)createStatusContents
{
    ...
    NSString *instructions = ([OWS2FAManager.sharedManager is2FAEnabled]
            ? Localized(@"ENABLE_2FA_VIEW_STATUS_ENABLED_INSTRUCTIONS",
                  @"Indicates that user has 'two factor auth pin' enabled.")
            : Localized(@"ENABLE_2FA_VIEW_STATUS_DISABLED_INSTRUCTIONS",
                  @"Indicates that user has 'two factor auth pin' disabled."));
    UILabel *instructionsLabel = [self createLabelWithText:instructions];

    [self createTableView];

    [instructionsLabel autoPinEdgeToSuperviewSafeArea:ALEdgeTop withInset:kVSpacing];
    ...
    [self.tableViewController.view autoPinEdgeToSuperviewSafeArea:ALEdgeBottom];
...
{
    [self createEnterPINContentsWithInstructions:Localized(@"ENABLE_2FA_VIEW_SELECT_PIN_INSTRUCTIONS",
...
{
    [self
        createEnterPINContentsWithInstructions:Localized(@"ENABLE_2FA_VIEW_CONFIRM_PIN_INSTRUCTIONS",
            ...
            if ([OWS2FAManager.sharedManager is2FAEnabled]) {
                [section
                    addItem:[OWSTableItem disclosureItemWithText:
                                              Localized(@"ENABLE_2FA_VIEW_DISABLE_2FA",
                ...
                [section
                    addItem:[OWSTableItem disclosureItemWithText:
                                              Localized(@"ENABLE_2FA_VIEW_ENABLE_2FA",
                                                  @"Label for the 'enable two-factor auth' item in the settings view")
                                                     actionBlock:^{
...
- (void)updateNavigationItems
{
    ...
    self.navigationItem.backBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:Localized(@"BACK_BUTTON", @"button text for back button")
    ...
    if (self.shouldHaveNextButton) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
            initWithTitle:Localized(@"ENABLE_2FA_VIEW_NEXT_BUTTON",
        ...
        case OWS2FASettingsMode_ConfirmPIN: {
            ...
            if ([self.pinTextfield.text isEqualToString:self.candidatePin]) {
                ...
                [OWSAlerts showErrorAlertWithMessage:
                               Localized(@"ENABLE_2FA_VIEW_PIN_DOES_NOT_MATCH",
                      ...
                      [OWS2FAManager.sharedManager disable2FAWithSuccess:^{
                          ...
                          failure:^(NSError *error) {
                              [modalActivityIndicator dismissWithCompletion:^{
                                  ...
                                  [OWSAlerts showErrorAlertWithMessage:
                                                 Localized(@"ENABLE_2FA_VIEW_COULD_NOT_DISABLE_2FA",
                      ...
                      [OWS2FAManager.sharedManager requestEnable2FAWithPin:self.candidatePin
                          ...
                          failure:^(NSError *error) {
                              [modalActivityIndicator dismissWithCompletion:^{
                                  ...
                                  [OWSAlerts showErrorAlertWithMessage:
                                                 Localized(@"ENABLE_2FA_VIEW_COULD_NOT_ENABLE_2FA",
