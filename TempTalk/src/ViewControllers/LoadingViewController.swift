...
public class LoadingViewController: UIViewController {
    ...
    @objc
    public func refreshOverlayTitle() {
        guard let titleLable = self.topLabel else {
            return
        }
        if DTDBKeyManager.sharedInstance().fetchingDBKey {
            titleLable.text = Localized("FETCH_DB_KEY_OVERLAY_TITLE", comment: "")
        } else {
            titleLable.text = Localized("DATABASE_VIEW_OVERLAY_TITLE", comment: "Title shown while the app is updating its database.")
        }
    }
}
