...
- (void)prepareUIdata {
    ...
    globalNotification = self.contact.privateConfigs.globalNotification;
...
- (void)setProfileWithGlobalNotification:(NSNumber *)globalNotification {
    OWSLogInfo(@"putV1ProfileWithGlobalNotification %@.",globalNotification);
    [DTToastHelper showHudInView:self.view];
    
    NSDictionary *parms = @{@"privateConfigs": @{@"globalNotification": globalNotification}};
    ...
    [self.networkManager makeRequest:request success:^(id<HTTPResponse>  _Nonnull response) {
        ...
        if (DTParamsUtils.validateDictionary(responseObject)) {
            ...
            if (DTParamsUtils.validateNumber(status) && [status intValue] == 0) {
                ...
                DatabaseStorageWrite(self.databaseStorage, ^(SDSAnyWriteTransaction *transation) {
                    OWSContactsManager *contactsManager = Environment.shared.contactsManager;
                    NSString *recipientId = [[TSAccountManager sharedInstance] localNumberWithTransaction:transation];
                    SignalAccount *account = [contactsManager signalAccountForRecipientId:recipientId transaction:transation];
...
- (DTGlobalNotificationType)getGlobalNotificationType {
    if (!self.globalNotification) return DTGlobalNotificationTypeMENTION;
