...
public class OWS2FAReminderViewController: UIViewController, PinEntryViewDelegate {
    ...
    override public func loadView() {
        ...
        self.navigationItem.title = Localized("REMINDER_2FA_NAV_TITLE", comment: "Navbar title for when user is periodically prompted to enter their registration lock PIN")
    ...
    public func pinEntryViewForgotPinLinkTapped(_ entryView: PinEntryView) {
        Logger.info("\(logTag) in \(#function)")
        let alertBody = Localized("REMINDER_2FA_FORGOT_PIN_ALERT_MESSAGE",
    ...
    private func didSubmitWrongPin() {
        ...
        Logger.info("\(logTag) in \(#function)")
        let alertTitle = Localized("REMINDER_2FA_WRONG_PIN_ALERT_TITLE",
