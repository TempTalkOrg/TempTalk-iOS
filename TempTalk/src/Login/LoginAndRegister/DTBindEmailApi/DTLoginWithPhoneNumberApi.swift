//
//  DTLoginWithPhoneNumberApi.swift
//  Signal
//
...
import Foundation
@objc
public class DTLoginWithPhoneNumberApi : DTBaseAPI {
    ...
    @objc
    func login(_ phone: String, dialingCode: String?, sucess: @escaping RESTNetworkManagerSuccess, failure:((Error, DTAPIMetaEntity?) -> Void)? = nil)  {
        ...
        self.networkManager.makeRequest(request, success: { response in
            do {
                ...
                if(entity.status != 0){
                    ...
                    let error = NSError(domain: "/v2/auth/login/sms/send error", code: entity.status ,userInfo: [NSLocalizedDescriptionKey:entity.reason])
                    failure(error,nil)
                } else {
                    sucess(response)
                }
