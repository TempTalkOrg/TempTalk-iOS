//
//  DTVerificationPhoneCodeApi.swift
//  Signal
//
...
@objc
public class DTVerificationPhoneCodeApi : DTBaseAPI {
    ...
    public override var requestUrl: String {
        get {
            return "/v2/auth/bind/sms/verification";
        }
        set{
    ...
    @objc
    func verification(_ phone: String, code: String, nonce: String?, sucess:@escaping (DTAPIMetaEntity?) -> Void, failure:((Error, DTAPIMetaEntity?) -> Void)? = nil)  {
        var parms: [String : String] = ["phone": phone, "verificationCode":code]
        if let nonce,
           !nonce.isEmpty {
            parms["nonce"] = nonce
        }
        ...
        self.networkManager.makeRequest(request) { response in
            do {
                ...
                if(entity.status != 0){
                    ...
                    let error = NSError(domain: "/v2/auth/bind/verification", code: entity.status ,userInfo: [NSLocalizedDescriptionKey:entity.reason])
                    failure(error,nil)
                } else {
                    sucess(entity)
                }
