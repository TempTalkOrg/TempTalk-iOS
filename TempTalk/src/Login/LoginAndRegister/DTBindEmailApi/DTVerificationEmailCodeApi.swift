//
//  DTVerificationEmailCodeApi.swift
//  Signal
//
...
@objc
public class DTVerificationEmailCodeApi : DTBaseAPI {
    ...
    public override var requestUrl: String {
        get {
            return "/v2/auth/bind/verification";
        }
        set{
    ...
    @objc
    func verificationCode(_ code: String, nonce: String?, sucess:@escaping RESTNetworkManagerSuccess, failure:((Error, DTAPIMetaEntity?) -> Void)? = nil)  {
        var parms: [String : String] = ["verificationCode":code]
        if let nonce,
           !nonce.isEmpty {
            parms["nonce"] = nonce
        }
