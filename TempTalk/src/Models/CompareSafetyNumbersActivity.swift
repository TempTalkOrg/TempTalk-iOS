...
let CompareSafetyNumbersActivityType = "org.difft.wea.activity.CompareSafetyNumbers"
...
class CompareSafetyNumbersActivity: UIActivity {
    ...
    override var activityTitle: String? {
        get {
            return Localized("COMPARE_SAFETY_NUMBER_ACTION", comment: "Activity Sheet label")
        }
    }
    ...
    override func perform() {
        ...
        guard (pasteboardString != nil && pasteboardString!.count == 60) else {
            ...
            let error = OWSErrorWithCodeDescription(OWSErrorCode.userError,
                                                    Localized("PRIVACY_VERIFICATION_FAILED_NO_SAFETY_NUMBERS_IN_CLIPBOARD", comment: "Alert body for user error"))
        ...
        if pasteboardSafetyNumbers == mySafetyNumbers {
            ...
            let error = OWSErrorWithCodeDescription(OWSErrorCode.privacyVerificationFailure,
                                                    Localized("PRIVACY_VERIFICATION_FAILED_MISMATCHED_SAFETY_NUMBERS_IN_CLIPBOARD", comment: "Alert body"))
            delegate.compareSafetyNumbersActivity(self, failedWithError: error)
        }
    }
