...
@objc class AudioWaveformProgressView: UIView {
    ...
    @objc public var cachedAudioDuration: TimeInterval {
        didSet {
            redrawSamples()
        }
    }
    ...
    public var canScrub: Bool {
        return audioWaveform != nil || waveformWaitingTask == nil && cachedAudioDuration != 0
    }
    ...
    @objc public init(waveform: AudioWaveform) {
    ...
    private func redrawSamples() {
        ...
        let minSampleSpacing: CGFloat = 4
        ...
        } else if cachedAudioDuration != 0 && waveformWaitingTask == nil {
...
//        Logger.debug("[voice] width: \(width), progress: \(progress),  thumbXPos: \(thumbXPos)")
        ...
        for (x, sample) in amplitudes.enumerated() {
            ...
            let sampleHeight = x == playedLines ? minSampleHeight : max(minSampleHeight, height * CGFloat(sample))
