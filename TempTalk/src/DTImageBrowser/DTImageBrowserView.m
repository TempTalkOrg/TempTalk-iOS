...
#ifndef DT_CLAMP // return the clamped value
#define DT_CLAMP(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))
#endif
extern const CGFloat kAvatarSize;
static inline CGSize CGSizeFromPixelCeil(CGSize size) {
    CGFloat scale = [UIScreen mainScreen].scale;
    return CGSizeMake(ceil(size.width * scale) / scale,
                      ceil(size.height * scale) / scale);
}

@interface DTImageViewModel()<NSCopying>
@property (nonatomic, readonly) UIImage *thumbImage;
@property (nonatomic, readonly) BOOL thumbClippedToTop;
- (BOOL)shouldClipToTop:(CGSize)imageSize forView:(UIView *)view;
@end
@implementation DTImageViewModel
...
- (id)copyWithZone:(NSZone *)zone {
    DTImageViewModel *viewModel = [self.class new];
    return viewModel;
}
@end

@interface DTImageGroupCell : UIScrollView <UIScrollViewDelegate>
@property (nonatomic, strong) UIView *imageContainerView;
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, assign) NSInteger page;

@property (nonatomic, strong) DTImageViewModel *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation DTImageGroupCell
...
- (void)setItem:(DTImageViewModel *)item {
    ...
    if (item.thread.isGroupThread) {
        ...
        if (item.image) {
            ...
            [_imageView setImageWithGroupThread:(TSGroupThread *)item.thread diameter:(NSUInteger)kAvatarSize contactsManager:contactsManager completion:^(UIImage * _Nonnull image) {
                @strongify(self);
                if (image) {
                    UIImage *cropedImage = [self cropSquareImage:image];
        ...
        if ([[item.thread serverThreadId] isEqualToString:[TSAccountManager sharedInstance].localNumber]) {
            _imageView.image = [UIImage imageNamed:@"icon_note_to_self"];
            self.maximumZoomScale = 3;
            ...
            [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction * _Nonnull transaction) {
                [_imageView setImageWithContactAvatar:item.avatar
                                          recipientId:item.receptid
...
@interface DTImageBrowserView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation DTImageBrowserView
...
- (void)updateCurrentItemWith:(DTImageViewModel *)viewModel {
    DTImageGroupCell *cell = [self cellForPage:self.currentPage];
    cell.item = viewModel;
}
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((DTImageViewModel *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    self.pager.alpha = 0;
    self.pager.numberOfPages = (NSInteger)self.groupItems.count;
    ...
    DTImageGroupCell *cell = [self cellForPage:self.currentPage];
    DTImageViewModel *item = _groupItems[(NSUInteger)self.currentPage];
    if (!cell.item) {
        cell.item = item;
    ...
    if (item.thumbClippedToTop) {
        ...
        self.fromViewFrame = fromFrame;
        OWSLogInfo(@"fromView::::%p; fromFrame = %@; cell.imageContainerView.frame = %@",fromView,NSStringFromCGRect(fromFrame),NSStringFromCGRect(cell.imageContainerView.frame));
        cell.imageContainerView.clipsToBounds = NO;
        cell.imageView.frame = fromFrame;
        cell.imageView.contentMode = UIViewContentModeScaleAspectFill;
        
        double oneTime = animated ? 0.18 : 0;
        ...
        cell.imageView.layer.cornerRadius = fromView.bounds.size.width/2.0;
        [UIView animateWithDuration:oneTime delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^ __nullable)(void))completion {
    ...
    NSInteger currentPage = (NSInteger)self.currentPage;
    DTImageGroupCell *cell = [self cellForPage:currentPage];
    DTImageViewModel *item = _groupItems[(NSUInteger)currentPage];
    ...
    [UIView animateWithDuration:animated ? 0.2 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (isFromImageClipped) {
            CGRect fromFrame = self.fromViewFrame;
            ...
            CGRect fromFrame = self.fromViewFrame;
            OWSLogInfo(@"fromView::::%p fromFrame = %@;  frame = %@;  imageContainerView.frame =  %@ ",fromView,NSStringFromCGRect(fromFrame),NSStringFromCGRect(fromView.frame),NSStringFromCGRect(cell.imageContainerView.frame));
            cell.imageContainerView.clipsToBounds = NO;
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(DTImageGroupCell * cell, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    NSInteger page = (NSInteger)(_scrollView.contentOffset.x / _scrollView.width + 0.5);
    
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < (NSInteger)self.groupItems.count) {
            DTImageGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                cell = [self dequeueReusableCell];
                cell.page = i;
                cell.left = (self.width + kPadding) * i + kPadding / 2;
                if (self->_isPresented) {
                    cell.item = self.groupItems[(NSUInteger)i];
                }
    ...
    NSInteger intPage = (NSInteger)(floatPage + 0.5);
    intPage = intPage < 0 ? 0 : intPage >= (NSInteger)_groupItems.count ? (int)_groupItems.count - 1 : intPage;
...
- (void)updateCellsForReuse {
    for (DTImageGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (DTImageGroupCell *)dequeueReusableCell {
    DTImageGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (DTImageGroupCell *)cellForPage:(NSInteger)page {
    for (DTImageGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (NSInteger)currentPage {
    NSInteger page = (NSInteger)(_scrollView.contentOffset.x / _scrollView.width + 0.5);
    if (page >= (NSInteger)_groupItems.count) page = (NSInteger)_groupItems.count - 1;
    if (page < 0) page = 0;
...
- (void)showHUD:(NSString *)msg {
    ...
    label.size = CGSizeFromPixelCeil(size);
    label.font = font;
    label.text = msg;
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    DTImageGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                duration *= 0.8;
                duration = DT_CLAMP(duration, 0.05, 0.3);
