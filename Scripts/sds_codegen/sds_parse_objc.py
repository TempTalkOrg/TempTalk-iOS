...
def process_objc_category(namespace, file_path, lines, decl_prefix, decl_remainder):
    ...
    # `-ObjCCategoryImplDecl 0x7fe6120af998 <line:35:1, line:493:1> line:35:17 invalid ChatFolder
...
#| |-ObjCPropertyImplDecl 0x7fde28d6c3c0 <line:65:1, col:10> col:10 onePageMsgs dynamic
#modify by Kris
process_objc_property_impl_regex = re.compile(r"^.+ ([^ ]+) (synthesize|dynamic)$")
...
def process_objc_property(clazz, prefix, file_path, line, remainder):
    ...
    if property is None:
        ...
        if property.objc_type != property_type:
            ...
            if (property_type.startswith('NSMutable') and property.objc_type == 'NS' + property_type[len('NSMutable'):]) or (property_type == ('BOOL') and property.objc_type == 'int') or (property_type == ('uint64_t') and property.objc_type == 'int') or (property_type == ('NSString *') and property.objc_type == 'int *'):
...
def process_objc(file_path: str, swift_bridging_path: str) -> None:
    ...
    command = [
        ...
        '-I/Users/user/work/Projects/difft/difft-ios-grdbRoot/Pods/SignalCoreKit/SignalCoreKit',
        '-include',
        pch_include,
        file_path,
        # '|',
...
def process_file(file_path, swift_bridging_path):
    ...
    # TODO: Fix this file 
    # kris
    if filename in ['OWSDisappearingMessageFinderTest.m', 'HomeViewController+ChatFolder.m', 'ConversationViewController.m', 'OWSMessageTranslateView.m', 'TSCall.m', 'OWSMessageSender.m', 'DTThreadHelper.m', 'OWSLinkedDevicesTableViewController.m', 'OWSLinkDeviceViewController.m', 'OWSQRCodeScanningViewController.m', 'WeaCallKitManager.m', 'UIImageView+ContactAvatar.m', 'AppVersion.m']: 
