...
//        let wasRegistered = TSAccountManager.shared.isRegistered
    ...
    public override func read(file: String = #file,
        ...
        guard storageCoordinatorState == .GRDB else {
            owsFailDebug("Yap unsupportted read.")
            return
        }
...
//        guard storageCoordinatorState == .GRDB else {
//            owsFailDebug("Yap unsupportted write.")
//            return
//        }
//        write(file: "objc", function: "objc", line: 0, block: block)
//    }
    
    @objc
    public func uiRead(block: @escaping (SDSAnyReadTransaction) -> Void) {
        guard storageCoordinatorState == .GRDB else {
            owsFailDebug("Yap unsupportted uiRead.")
            return
        }
        read(file: "objc", function: "objc", line: 0, block: block)
    }
