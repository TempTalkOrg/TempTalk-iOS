...
public protocol DatabaseChanges: AnyObject {
    ...
    var threadDeletedUniqueIds: Set<UniqueId> { get }
...
class ObservedDatabaseChanges: NSObject {
    ...
    func append(threadDeletedUniqueIds: Set<UniqueId>) {
        #if TESTABLE_BUILD
        checkConcurrency()
        #endif

        threads.append(deletedUniqueIds: threadDeletedUniqueIds)
    }
...
extension ObservedDatabaseChanges: DatabaseChanges {
    ...
    var threadDeletedUniqueIds: Set<UniqueId> {
        #if TESTABLE_BUILD
        checkConcurrency()
        #endif

        return threads.deletedUniqueIds
    }
    ...
    private func mapTableNamesToCollections() {
        ...
        for tableName in tableNames {
            ...
            // FTS5 virtual table
            if tableName.hasPrefix("model_TSMessageSecondary_virtual") {
                continue
            }
