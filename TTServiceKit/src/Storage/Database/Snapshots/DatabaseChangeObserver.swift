...
public class DatabaseChangeObserver: NSObject {
    ...
    @objc
    public static let databaseDidCommitInteractionChangeNotification = Notification.Name("databaseDidCommitInteractionChangeNotification\(TSConstants.appDisplayName)")
...
//                                                        PendingReadReceiptRecord.databaseTableName
        TSInteraction.archivedTableName
    ])
...
extension DatabaseChangeObserver: TransactionObserver {
    ...
    func didTouch(interaction: TSInteraction, transaction: GRDBWriteTransaction) {
        ...
        if !pendingChanges.threadUniqueIds.contains(interaction.uniqueThreadId) {
            ...
            let interactionThread: TSThread? = interaction.thread(with: transaction.asAnyRead)
    ...
    public func databaseDidCommit(_ db: Database) {
        DatabaseChangeObserver.serializedSync {
            ...
            do {
                ...
                let threadDeletedUniqueIds = pendingChangesToCommit.threadDeletedUniqueIds
                ...
                Self.committedChangesLock.withLock {
                    ...
                    self.committedChanges.append(threadDeletedUniqueIds: threadDeletedUniqueIds)
    ...
    private func publishUpdatesIfNecessary() {
        ...
        //TODO Open annotation
//        guard  self.socketManager.socketState() == .open  else {
////            if tsAccountManager.isRegisteredAndReady {
////                OWSLogger.warn("Skipping publishing of updates; PullHotUnreadMessagesData in progress.")
////            }
//            return
//        }
        
        if self.socketManager.socketState() != .open, DatabaseOfflineManager.shared.canOfflineUpdateDatabase != true {
            return
        }
