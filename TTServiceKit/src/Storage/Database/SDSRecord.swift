...
//                owsFailDebug("Could not insert existing record.")
    ...
    private func sdsUpdate(grdbId: Int64, transaction: GRDBWriteTransaction) {
        do {
            ...
            if self is MessageReadPositionRecord {
                sdsProdError(saveMode: .update, eventName: "Update readPosition failed: \(error.grdbErrorForLogging)")
            } else {
    ...
    private func sdsProdError(
        ...
        Logger.error("\(function):\(line) saveMode: \(saveMode): \(eventName)")
        
        var file = String(describing: Self.self)
