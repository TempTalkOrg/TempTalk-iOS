...
public class GRDBDatabaseStorageAdapter: NSObject {
    ...
    public static func databaseFileUrl(directoryMode: DirectoryMode = .primary) -> URL {
        ...
        return databaseDir.appendingPathComponent("difft.sqlite", isDirectory: false)
    }

    public static func databaseWalUrl(directoryMode: DirectoryMode = .primary) -> URL {
        let databaseDir = databaseDirUrl(directoryMode: directoryMode)
        OWSFileSystem.ensureDirectoryExists(databaseDir.path)
        return databaseDir.appendingPathComponent("difft.sqlite-wal", isDirectory: false)
    }
    ...
    static var tables: [SDSTableMetadata] {
        [
            ...
            TSMessageReadPosition.table,
            DTReactionMessage.table,
            OWSRecipientIdentity.table,
            OWSDevice.table,
            DTCardMessageEntity.table
        ]
    }
    ...
    private static let keyServiceName: String = "TTGRDBKeyChainService"
    private static let keyName: String = "TTGRDBDatabaseCipherKeySpec"
    private static let legacyKeyServiceName: String = "GRDBKeyChainService"
    private static let legacyKeyName: String = "GRDBDatabaseCipherKeySpec"
    public static var keyspec: GRDBKeySpecSource {
        return GRDBKeySpecSource(keyServiceName: keyServiceName, keyName: keyName)
    }
    
    public static var legacyKeyspec: GRDBKeySpecSource {
        return GRDBKeySpecSource(keyServiceName: legacyKeyServiceName, keyName: legacyKeyName)
    }
    ...
    @objc
    public static func runKeyspecMigrations() {
        ...
        guard doesDBExist else {
            return
        }
        OWSLogger.info("run keyspec migrations.")
        var dataEmpty = false
        do {
            _ = try keyspec.fetchData()
        } catch KeychainStorageError.missing {
            dataEmpty = true
        } catch {
            OWSLogger.error("keyspec migrations fetchData error: \(error).")
        }
        do {
            let legacyData = try legacyKeyspec.fetchData()
            if !legacyData.isEmpty,
               dataEmpty {
                try keyspec.store(data: legacyData)
                OWSLogger.info("complete keyspec migrations.")
            }
        } catch {
            OWSLogger.error("keyspec migrations error: \(error).")
        }
    }
    ...
    static func prepareDatabase(db: Database, keyspec: GRDBKeySpecSource, name: String? = nil) throws {
        ...
        let keyspecString = try keyspec.fetchString()
        Logger.debug("------ keyspecString:\(keyspecString)")
        try db.execute(sql: "PRAGMA \(prefix)key = \"\(keyspecString)\"")
