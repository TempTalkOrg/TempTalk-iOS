...
#define DifftMBMainThreadAssert() NSAssert([NSThread isMainThread], @"DifftMBProgressHUD needs to be accessed on the main thread.");

CGFloat const DifftMBProgressMaxOffset = 1000000.f;

static const CGFloat DifftMBDefaultPadding = 4.f;
static const CGFloat DifftMBDefaultLabelFontSize = 16.f;
static const CGFloat DifftMBDefaultDetailsLabelFontSize = 12.f;


@interface DifftMBProgressHUD ()
...
@interface DifftMBProgressHUDRoundedButton : UIButton
@end


@implementation DifftMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    DifftMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    DifftMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (DifftMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            DifftMBProgressHUD *hud = (DifftMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = DifftMBProgressHUDAnimationFade;
    _mode = DifftMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(DifftMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == DifftMBProgressHUDAnimationZoom) {
        type = animatingIn ? DifftMBProgressHUDAnimationZoomIn : DifftMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == DifftMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == DifftMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == DifftMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == DifftMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    DifftMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<DifftMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    DifftMBBackgroundView *backgroundView = [[DifftMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = DifftMBProgressHUDBackgroundStyleSolidColor;
    ...
    DifftMBBackgroundView *bezelView = [DifftMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:DifftMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:DifftMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [DifftMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:DifftMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[DifftMBRoundProgressView class]];

    DifftMBProgressHUDMode mode = self.mode;
    if (mode == DifftMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == DifftMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[DifftMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == DifftMBProgressHUDModeDeterminate || mode == DifftMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[DifftMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == DifftMBProgressHUDModeAnnularDeterminate) {
            [(DifftMBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == DifftMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == DifftMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[DifftMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[DifftMBProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[DifftMBRoundProgressView class]]) {
        DifftMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [DifftMBRoundProgressView appearanceWhenContainedIn:[DifftMBProgressHUD class], nil];
#else
        appearance = [DifftMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[DifftMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((DifftMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((DifftMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[DifftMBBarProgressView class]]) {
        DifftMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [DifftMBBarProgressView appearanceWhenContainedIn:[DifftMBProgressHUD class], nil];
#else
        appearance = [DifftMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[DifftMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((DifftMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((DifftMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    DifftMBBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoDifftMBie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? DifftMBDefaultPadding : 0.f;
...
- (void)setMode:(DifftMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == DifftMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuDifftMBer >= kCFCoreFoundationVersionNuDifftMBer_iOS_8_0;
...
@implementation DifftMBRoundProgressView
...
@implementation DifftMBBarProgressView
...
@interface DifftMBBackgroundView ()
...
@implementation DifftMBBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = DifftMBProgressHUDBackgroundStyleBlur;
        #if TARGET_OS_TV
...
- (void)setStyle:(DifftMBProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    DifftMBProgressHUDBackgroundStyle style = self.style;
    if (style == DifftMBProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == DifftMBProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation DifftMBProgressHUDRoundedButton
