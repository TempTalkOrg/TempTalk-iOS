...
public struct LinkedDeviceReadReceiptRecord: SDSRecord {
    ...
    public var tableMetadata: SDSTableMetadata {
        OWSLinkedDeviceReadReceiptSerializer.table
    }

    public static var databaseTableName: String {
        OWSLinkedDeviceReadReceiptSerializer.table.tableName
    }
    ...
    public let isLargeGroupThread: Bool
    public let messageIdTimestamp: UInt64
    public let readPosition: Data
...
public extension LinkedDeviceReadReceiptRecord {
    ...
    init(row: Row) {
        ...
        isLargeGroupThread = row[4]
        messageIdTimestamp = row[5]
        readPosition = row[6]
        readTimestamp = row[7]
        senderId = row[8]
        whisperMessageType = row[9]
        messageModeType = row[10]
    }
}
...
extension OWSLinkedDeviceReadReceipt {
    ...
    class func fromRecord(_ record: LinkedDeviceReadReceiptRecord) throws -> OWSLinkedDeviceReadReceipt {
        ...
        case .linkedDeviceReadReceipt:
            ...
            let isLargeGroupThread: Bool = record.isLargeGroupThread
            let messageIdTimestamp: UInt64 = record.messageIdTimestamp
            let messageModeType: TSMessageModeType = record.messageModeType
            let readPositionSerialized: Data = record.readPosition
            let readPosition: DTReadPositionEntity = try SDSDeserialization.unarchive(readPositionSerialized, name: "readPosition")
            let readTimestamp: UInt64 = record.readTimestamp
            let senderId: String = record.senderId
            let whisperMessageType: TSWhisperMessageType = record.whisperMessageType

            return OWSLinkedDeviceReadReceipt(grdbId: recordId,
                                              uniqueId: uniqueId,
                                              associatedUniqueThreadId: associatedUniqueThreadId,
                                              isLargeGroupThread: isLargeGroupThread,
...
extension OWSLinkedDeviceReadReceipt: SDSModel {
    ...
    public var sdsTableName: String {
        LinkedDeviceReadReceiptRecord.databaseTableName
    }
...
extension OWSLinkedDeviceReadReceipt: DeepCopyable {
    ...
    public func deepCopy() throws -> AnyObject {
        ...
        do {
            ...
            assert(type(of: modelToCopy) == OWSLinkedDeviceReadReceipt.self)
            let uniqueId: String = modelToCopy.uniqueId
            let associatedUniqueThreadId: String = modelToCopy.associatedUniqueThreadId
            let isLargeGroupThread: Bool = modelToCopy.isLargeGroupThread
            let messageIdTimestamp: UInt64 = modelToCopy.messageIdTimestamp
            ...
            let readPosition: DTReadPositionEntity = try DeepCopies.deepCopy(modelToCopy.readPosition)
            let readTimestamp: UInt64 = modelToCopy.readTimestamp
            let senderId: String = modelToCopy.senderId
            let whisperMessageType: TSWhisperMessageType = modelToCopy.whisperMessageType

            return OWSLinkedDeviceReadReceipt(grdbId: id,
                                              uniqueId: uniqueId,
                                              associatedUniqueThreadId: associatedUniqueThreadId,
                                              isLargeGroupThread: isLargeGroupThread,
                                              messageIdTimestamp: messageIdTimestamp,
...
extension OWSLinkedDeviceReadReceiptSerializer {
    ...
    static var isLargeGroupThreadColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "isLargeGroupThread", columnType: .int) }
    static var messageIdTimestampColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "messageIdTimestamp", columnType: .int64) }
    static var readPositionColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "readPosition", columnType: .blob) }
    static var readTimestampColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "readTimestamp", columnType: .int64) }
    static var senderIdColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "senderId", columnType: .unicodeString) }
    static var whisperMessageTypeColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "whisperMessageType", columnType: .int) }
    static var messageModeTypeColumn: SDSColumnMetadata { SDSColumnMetadata(columnName: "messageModeType", columnType: .int) }

    // TODO: We should decide on a naming convention for
    //       tables that store models.
    public static var table: SDSTableMetadata {
        SDSTableMetadata(collection: OWSLinkedDeviceReadReceipt.collection(),
...
@objc
public class OWSLinkedDeviceReadReceiptCursor: NSObject, SDSCursor {
    private let transaction: GRDBReadTransaction
    private let cursor: RecordCursor<LinkedDeviceReadReceiptRecord>?

    init(transaction: GRDBReadTransaction, cursor: RecordCursor<LinkedDeviceReadReceiptRecord>?) {
...
public extension OWSLinkedDeviceReadReceipt {
    class func grdbFetchCursor(transaction: GRDBReadTransaction) -> OWSLinkedDeviceReadReceiptCursor {
        ...
        do {
            ...
            return OWSLinkedDeviceReadReceiptCursor(transaction: transaction, cursor: cursor)
        } catch {
            owsFailDebug("Read failed: \(error)")
            return OWSLinkedDeviceReadReceiptCursor(transaction: transaction, cursor: nil)
        }
    }
...
class OWSLinkedDeviceReadReceiptSerializer: SDSSerializer {
    ...
    func asRecord() throws -> SDSRecord {
        ...
        let isLargeGroupThread: Bool = model.isLargeGroupThread
        let messageIdTimestamp: UInt64 = model.messageIdTimestamp
        let readPosition: Data = requiredArchive(model.readPosition)
        ...
        return LinkedDeviceReadReceiptRecord(delegate: model, id: id, recordType: recordType, uniqueId: uniqueId, associatedUniqueThreadId: associatedUniqueThreadId, isLargeGroupThread: isLargeGroupThread, messageIdTimestamp: messageIdTimestamp, readPosition: readPosition, readTimestamp: readTimestamp, senderId: senderId, whisperMessageType: whisperMessageType, messageModeType: messageModeType)
    }
}
...
public extension OWSLinkedDeviceReadReceipt {
    ...
    func deepCopyUsingRecord() throws -> OWSLinkedDeviceReadReceipt {
        guard let record = try asRecord() as? LinkedDeviceReadReceiptRecord else {
