...
//        }
    ...
    // MARK: - did ever has some apps
    
    private static var didEverHasWXKey: String { "didEverHasWX" }
    
    public static func didEverHasWX() -> Bool {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        guard let preference = appUserDefaults.object(forKey: didEverHasWXKey) as? NSNumber else {
            return false
        }
        return preference.boolValue
    }
    
    public static func setDidEverHasWX(_ value: Bool) {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        appUserDefaults.set(value, forKey: didEverHasWXKey)
        appUserDefaults.synchronize()
    }
    
    private static var didEverHasDYKey: String { "didEverHasDY" }
    
    public static func didEverHasDY() -> Bool {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        guard let preference = appUserDefaults.object(forKey: didEverHasDYKey) as? NSNumber else {
            return false
        }
        return preference.boolValue
    }
    
    public static func setDidEverHasDY(_ value: Bool) {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        appUserDefaults.set(value, forKey: didEverHasDYKey)
        appUserDefaults.synchronize()
    }
    
    private static var didEverHasDYIKey: String { "didEverHasDYI" }
    
    public static func didEverHasDYI() -> Bool {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        guard let preference = appUserDefaults.object(forKey: didEverHasDYIKey) as? NSNumber else {
            return false
        }
        return preference.boolValue
    }
    
    public static func setDidEverHasDYI(_ value: Bool) {
        let appUserDefaults = CurrentAppContext().appUserDefaults()
        appUserDefaults.set(value, forKey: didEverHasDYIKey)
        appUserDefaults.synchronize()
    }
