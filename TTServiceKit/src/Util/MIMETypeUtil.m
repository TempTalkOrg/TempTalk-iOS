//
//  Copyright (c) 2022 Difft. All rights reserved.
//
...
+ (NSDictionary<NSString *, NSArray<NSString *> *> *)fileIconToMIMETypesMap {
    return @{
        @"zip":@[@"zip",
                 @"gzip",
                 @"bzip2",
...
+ (BOOL)isVoiceMessage:(DSKProtoAttachmentPointer *)attachmentProto {
    if ([attachmentProto hasFlags]) {
        ...
        if ((flags & (UInt32)DSKProtoAttachmentPointerFlagsVoiceMessage) > 0) {
            return YES;
        }
    }
