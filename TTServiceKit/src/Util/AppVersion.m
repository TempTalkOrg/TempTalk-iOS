...
//    OWSLogInfo(@"iOS Version: %@", [[self class] iOSVersionString]);
    ...
    float utc = timeZone.secondsFromGMT / 3600.f;
    OWSLogInfo(@"system timezone: %.2f", utc);
    ...
    OWSLogInfo(@"Device Model: %@", [self hardwareInfoString]);
    ...
    OWSLogInfo(@"Difft Commit: %@", buildDetails[@"DifftCommit"]);
