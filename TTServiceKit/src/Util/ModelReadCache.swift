...
struct ModelReadCacheStats {
    ...
    let cacheHitCount = AtomicUInt(lock: .sharedGlobal)
    let cacheReadCount = AtomicUInt(lock: .sharedGlobal)
...
public class UserProfileReadCache: NSObject {
    ...
    private class Adapter: ModelCacheAdapter<KeyType, ValueType> {
        override func read(key: KeyType, transaction: SDSAnyReadTransaction) -> ValueType? {
            OWSUserProfile.anyFetch(uniqueId: key, transaction: transaction, ignoreCache: true)
        }
    ...
    @objc
    public func getUserProfile(address: String, transaction: SDSAnyReadTransaction) -> OWSUserProfile? {
    ...
    public func getUserProfiles(for addresses: AnySequence<String>,
...
public class SignalAccountReadCache: NSObject {
    ...
    private class Adapter: ModelCacheAdapter<KeyType, ValueType> {
        ...
        override func read(key: KeyType, transaction: SDSAnyReadTransaction) -> ValueType? {
            SignalAccount.anyFetch(uniqueId: key, transaction: transaction, ignoreCache: true)
        }
    ...
    private let cache: ModelReadCache<KeyType, ValueType>
    private let adapter = Adapter(cacheName: "SignalAccount", cacheCountLimit: 1024, cacheCountLimitNSE: 32)
    ...
    @objc
    public func getSignalAccount(address: String, transaction: SDSAnyReadTransaction) -> SignalAccount? {
        let cacheKey = adapter.cacheKey(forKey: address)
        return cache.getValue(for: cacheKey, transaction: transaction)
    }

    public func getSignalAccounts(addresses: AnySequence<String>,
...
public class SignalRecipientReadCache: NSObject {
    ...
    private class Adapter: ModelCacheAdapter<KeyType, ValueType> {
        override func read(key: KeyType, transaction: SDSAnyReadTransaction) -> ValueType? {
            SignalRecipient.anyFetch(uniqueId: key, transaction: transaction, ignoreCache: true)
        }
    ...
    private let cache: ModelReadCache<KeyType, ValueType>
    private let adapter = Adapter(cacheName: "SignalRecipient", cacheCountLimit: 1000, cacheCountLimitNSE: 32)
    ...
    public func getSignalRecipient(address: String, transaction: SDSAnyReadTransaction) -> SignalRecipient? {
        let cacheKey = adapter.cacheKey(forKey: address)
        return cache.getValue(for: cacheKey, transaction: transaction)
    }
...
public class ThreadReadCache: NSObject {
    ...
    private let cache: ModelReadCache<KeyType, ValueType>
    private let adapter = Adapter(cacheName: "TSThread", cacheCountLimit: 1000, cacheCountLimitNSE: 8)
    ...
    @objc(getThreadIfInCacheForUniqueId:transaction:)
    public func getThreadIfInCache(forUniqueId uniqueId: String, transaction: SDSAnyReadTransaction) -> TSThread? {
        let cacheKey = adapter.cacheKey(forKey: uniqueId)
        return cache.getValue(for: cacheKey, transaction: transaction, returnNilOnCacheMiss: true)
    }
...
public class AttachmentReadCache: NSObject {
    ...
    private let cache: ModelReadCache<KeyType, ValueType>
    private let adapter = Adapter(cacheName: "TSAttachment", cacheCountLimit: 128, cacheCountLimitNSE: 8)
