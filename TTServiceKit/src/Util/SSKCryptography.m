...
@implementation SSKAES256Key

+ (nullable instancetype)keyWithData:(NSData *)data
{
    if (data.length != kAES256_KeyByteLength) {
        OWSFailDebug(@"Invalid key length for AES128: %lu", (unsigned long)data.length);
        return nil;
    }
...
- (instancetype)init
{
    return [self initWithData:[SSKCryptography generateRandomBytes:kAES256_KeyByteLength]];
}
...
@implementation SSKCryptography
...
+ (nullable NSString *)truncatedSHA1Base64EncodedWithoutPadding:(NSString *)string
{
    ...
    if (!stringData) {
        OWSFailDebug(@"%@ could not convert string to utf-8.", self.logTag);
        return nil;
    }
    if (stringData.length >= UINT32_MAX) {
        OWSFailDebug(@"%@ string data is too long.", self.logTag);
        return nil;
    }
...
+ (nullable NSData *)computeSHA256Digest:(NSData *)data truncatedToBytes:(NSUInteger)truncatedBytes
{
    if (data.length >= UINT32_MAX) {
        OWSFailDebug(@"%@ data is too long.", self.logTag);
        return nil;
    }
...
+ (nullable NSData *)computeSHA512Digest:(NSData *)data
{
    ...
    CC_SHA512(data.bytes, dataLength, digest);
...
+ (nullable NSData *)computeSHA256HMAC:(NSData *)data withHMACKey:(NSData *)HMACKey
{
    ...
    if (HMACKey.length >= SIZE_MAX) {
        OWSFailDebug(@"%@ HMAC key is too long.", self.logTag);
        return nil;
    }
...
{
    return [[SSKCryptography computeSHA1HMAC:dataToHMAC withHMACKey:HMACKey] subdataWithRange:NSMakeRange(0, bytes)];
}

+ (nullable NSData *)truncatedSHA256HMAC:(NSData *)dataToHMAC withHMACKey:(NSData *)HMACKey truncation:(NSUInteger)bytes
{
    return [[SSKCryptography computeSHA256HMAC:dataToHMAC withHMACKey:HMACKey] subdataWithRange:NSMakeRange(0, bytes)];
}
 ...
 * @deprecated useMd5Hash 字段废弃，内部通过 digest 的长度判断本地生成 digest 使用 md5 还是 sha256
 *
                         ...
                         useMd5Hash:(BOOL)useMd5Hash
{
    ...
    if (hmacType == SSKHMACSHA1Truncated10Bytes) {
        ourHmacData = [SSKCryptography truncatedSHA1HMAC:dataToAuth withHMACKey:hmacKey truncation:10];
    } else if (hmacType == SSKHMACSHA256Truncated10Bytes) {
        ourHmacData = [SSKCryptography truncatedSHA256HMAC:dataToAuth withHMACKey:hmacKey truncation:10];
    } else if (hmacType == SSKHMACSHA256AttachementType) {
        ourHmacData =
            [SSKCryptography truncatedSHA256HMAC:dataToAuth withHMACKey:hmacKey truncation:HMAC256_OUTPUT_LENGTH];
    }
    ...
    if (digest) {
        ...
        NSData *_Nullable ourDigest = nil;
        if(digest.length == 16){
            ourDigest = [SSKCryptography computeMD5Digest:dataToAuth];
        }else{
            ourDigest = [SSKCryptography computeSHA256Digest:dataToAuth];
        }
...
+ (nullable NSData *)decryptAppleMessagePayload:(NSData *)payload withSignalingKey:(NSString *)signalingKeyString
{
    ...
    return
        [SSKCryptography decryptCBCMode:[NSData dataWithBytesNoCopy:ciphertext length:ciphertext_length freeWhenDone:YES]
                            ...
                            HMACType:SSKHMACSHA256Truncated10Bytes
                                 ...
                                 error:(NSError **)error
{
    if (digest.length <= 0) {
        ...
        *error = OWSErrorWithCodeDescription(OWSErrorCodeFailedToDecryptMessage,
            Localized(@"ERROR_MESSAGE_ATTACHMENT_FROM_OLD_CLIENT",
    ...
    if (([dataToDecrypt length] < AES_CBC_IV_LENGTH + HMAC256_OUTPUT_LENGTH) ||
        ...
        *error = OWSErrorWithCodeDescription(
            OWSErrorCodeFailedToDecryptMessage, Localized(@"ERROR_MESSAGE_INVALID_MESSAGE", @""));
        return nil;
    }
    ...
    NSData *_Nullable paddedPlainText = [SSKCryptography decryptCBCMode:encryptedAttachment
                                                                 key:encryptionKey
                                                                  IV:iv
                                                             version:nil
                                                             HMACKey:hmacKey
                                                            HMACType:SSKHMACSHA256AttachementType
                                                        matchingHMAC:hmac
                                                              digest:digest
                                                          useMd5Hash:useMd5Hash];
    if (!paddedPlainText) {
        OWSLogError(@"%@ couldn't decrypt attachment.", self.logTag);
        *error = OWSErrorWithCodeDescription(
            OWSErrorCodeFailedToDecryptMessage, Localized(@"ERROR_MESSAGE_INVALID_MESSAGE", @""));
        return nil;
...
+ (nullable NSData *)encryptAttachmentData:(NSData *)attachmentData
                                      eKey:(NSData * _Nullable)encryptionKey0
                                   hmacKey:(NSData * _Nullable)hmacKey0
                                ...
                                useMd5Hash:(BOOL)useMd5Hash
{
    ...
    NSData *iv            = [SSKCryptography generateRandomBytes:AES_CBC_IV_LENGTH];
    NSData *encryptionKey = encryptionKey0?:[SSKCryptography generateRandomBytes:AES_KEY_SIZE];
    NSData *hmacKey       = hmacKey0?:[SSKCryptography generateRandomBytes:HMAC256_KEY_LENGTH];
    ...
    *outKey = [attachmentKey copy];
    ...
    NSData *_Nullable hmac =
        [SSKCryptography truncatedSHA256HMAC:encryptedPaddedData withHMACKey:hmacKey truncation:HMAC256_OUTPUT_LENGTH];
    if (!hmac) {
        OWSFailDebug(@"%@ could not compute SHA 256 HMAC.", self.logTag);
        return nil;
    }
    ...
    if(useMd5Hash){
        digest = [self computeMD5Digest:encryptedPaddedData];
    }else{
        digest = [self computeSHA256Digest:encryptedPaddedData];
    }
...
+ (nullable NSData *)encryptAESGCMWithData:(NSData *)plaintext key:(SSKAES256Key *)key
{
    NSData *initializationVector = [SSKCryptography generateRandomBytes:kAESGCM256_IVLength];
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, (int)initializationVector.length, NULL) != kOpenSSLSuccess) {
        OWSFailDebug(@"%@ failed to set IV length", self.logTag);
        return nil;
    }
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, kAESGCM256_TagLength, authTag.mutableBytes) != kOpenSSLSuccess) {
        OWSFailDebug(@"%@ failed to write tag", self.logTag);
        return nil;
    }
...
{
    if ((encryptedData.length <= kAESGCM256_IVLength + kAESGCM256_TagLength)) {
        return nil;
    }
                                                       ...
                                                       key:(SSKAES256Key *)key
{
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, kAESGCM256_IVLength, NULL) != kOpenSSLSuccess) {
        OWSFailDebug(@"%@ failed to set key and iv while decrypting", self.logTag);
        return nil;
    }
    ...
    if (authTagFromEncrypt.length >= INT_MAX) {
        OWSFailDebug(@"%@ authTagFromEncrypt too large", self.logTag);
        return nil;
    }
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, (int)authTagFromEncrypt.length, (void *)authTagFromEncrypt.bytes)
        != kOpenSSLSuccess) {
        OWSFailDebug(@"%@ Failed to set auth tag in decrypt.", self.logTag);
        return nil;
    }
...
+ (void)seedRandom
{
    ...
    unsigned seed = [SSKCryptography randomUnsigned];
    srand(seed);
}
