...
- (BOOL)matchesLogicalFingerprintsData:(NSData *)data error:(NSError **)error
{
    ...
    @try {
        ...
        if ([exception.name isEqualToString:@"InvalidProtocolBuffer"]) {
            NSString *description = Localized(@"PRIVACY_VERIFICATION_FAILURE_INVALID_QRCODE", @"alert body");
    ...
    if (logicalFingerprints.version < OWSFingerprintScannableFormatVersion) {
        ...
        NSString *description
            = Localized(@"PRIVACY_VERIFICATION_FAILED_WITH_OLD_REMOTE_VERSION", @"alert body");
        *error = OWSErrorWithCodeDescription(OWSErrorCodePrivacyVerificationFailure, description);
        return NO;
    }
    ...
    if (logicalFingerprints.version > OWSFingerprintScannableFormatVersion) {
        ...
        NSString *description = Localized(@"PRIVACY_VERIFICATION_FAILED_WITH_OLD_LOCAL_VERSION", @"alert body");
        *error = OWSErrorWithCodeDescription(OWSErrorCodePrivacyVerificationFailure, description);
        return NO;
    }
    ...
    if (![remoteFingerprint.identityData isEqual:[self scannableData:self.theirFingerprintData]]) {
        ...
        NSString *descriptionFormat = Localized(@"PRIVACY_VERIFICATION_FAILED_I_HAVE_WRONG_KEY_FOR_THEM",
    ...
    if (![localFingerprint.identityData isEqual:[self scannableData:self.myFingerprintData]]) {
        ...
        NSString *descriptionFormat = Localized(@"PRIVACY_VERIFICATION_FAILED_THEY_HAVE_WRONG_KEY_FOR_ME",
            @"Alert body when verifying with {{contact name}}");
        NSString *description = [NSString stringWithFormat:descriptionFormat, self.theirName];
        *error = OWSErrorWithCodeDescription(OWSErrorCodePrivacyVerificationFailure, description);
        return NO;
...
- (nullable UIImage *)image
{
    FingerprintProtoLogicalFingerprintsBuilder *logicalFingerprintsBuilder =
        [FingerprintProtoLogicalFingerprints builder];
    ...
    FingerprintProtoLogicalFingerprintBuilder *remoteFingerprintBuilder =
        [FingerprintProtoLogicalFingerprint builder];

    remoteFingerprintBuilder.identityData = [self scannableData:self.theirFingerprintData];
    logicalFingerprintsBuilder.remoteFingerprint = [remoteFingerprintBuilder buildAndReturnError:nil];
    ...
    logicalFingerprintsBuilder.localFingerprint = [localFingerprintBuilder buildAndReturnError:nil];

    // Build ByteMode QR (Latin-1 encodable data)
    NSData *fingerprintData = [logicalFingerprintsBuilder buildSerializedDataAndReturnError:nil];
    ...
    if (!ciImage) {
        DDLogError(@"%@ Failed to create QR image from fingerprint", self.logTag);
        return nil;
    }
