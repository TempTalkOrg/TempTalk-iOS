//
//  UIImage+DTBlurImage.m
//  TTServiceKit
//
//  Created by hornet on 2022/7/21.
//

#import "UIImage+DTBlurImage.h"
#import <Accelerate/Accelerate.h>

#ifndef DT_SWAP // swap two value
#define DT_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif

// Helper function to handle deferred cleanup of a buffer.
static void _dt_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}


@implementation UIImage (DTBlurImage)
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _dt_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_dt_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _dt_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
- (UIImage *)_dt_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
