...
public class FailedMessagesJob: NSObject, Dependencies {
    ...
    public func runSync() {
        databaseStorage.write { writeTx in
            InteractionFinder.attemptingOutInteractionIds(transaction: writeTx).forEach { failedInteractionId in
                ...
                autoreleasepool {
                    self.updateFailedMessageIfNecessary(failedInteractionId, transaction: writeTx)
                }
            }
        }
    ...
    public func updateFailedMessageIfNecessary(_ uniqueId: String, transaction writeTx: SDSAnyWriteTransaction) {
        ...
        // Update
        message.updateWithAllSendingRecipientsMarkedAsFailed(withTansaction: writeTx)
        count += 1
