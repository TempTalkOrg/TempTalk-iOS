...
@objc public class AudioWaveform: NSObject {

    /// The recorded samples for this waveform.
    @objc public let decibelSamples: [Float]

    @objc public init(decibelSamples: [Float]) {
    ...
    @objc public init(archivedData: Data) throws {
        var unarchivedSamples: [NSNumber]?
        
        if #available(iOS 14.0, *) {
            unarchivedSamples = try NSKeyedUnarchiver.unarchivedArrayOfObjects(ofClass: NSNumber.self, from: archivedData)
        } else {
            unarchivedSamples = try NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSNumber.self], from: archivedData) as? [NSNumber]
        }
