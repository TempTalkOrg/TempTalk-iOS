...
@objc public enum OWSWebSocketState: Int {
    case closed
    case connecting
    case open
}
...
public class OWSWebSocket: NSObject {
    ...
    public static let serialQueue = DispatchQueue(label: "org.difft.websocket")
    fileprivate var serialQueue: DispatchQueue { Self.serialQueue }
    
    // TODO: Should we use a higher-priority queue?
    fileprivate static let messageProcessingQueue = DispatchQueue(label: "org.difft.websocket.messageProcessingQueue")
    ...
    private let lastNewWebsocketDate = AtomicOptional<Date>(nil, lock: .sharedGlobal)
    private let lastDrainQueueDate = AtomicOptional<Date>(nil, lock: .sharedGlobal)
    private let lastReceivedPushWithoutWebsocketDate = AtomicOptional<Date>(nil, lock: .sharedGlobal)
    ...
    private let hasObservedNotifications = AtomicBool(false, lock: .sharedGlobal)
    ...
    private let lastState = AtomicValue<OWSWebSocketState>(.closed, lock: .sharedGlobal)
    
    private func notifyStatusChange() {
        let newState = self.currentState
        let oldState = lastState.swap(newState)
        if oldState != newState {
            Logger.info("\(logPrefix): websocketState \(oldState.rawValue) -> \(newState.rawValue)")
        }
...
//        owsAssertDebug(!requestUrl.path.hasPrefix("/"))
        ...
        if requestPath.hasPrefix("/") == false {
            requestPath = "/".appending(requestPath)
        }
    ...
    private func processWebSocketResponseMessage(_ message: WebSocketProtoWebSocketResponseMessage,
        ...
        if hasSuccessStatus {
            ...
            if webSocketType == .identified {
                ...
                } else if responseStatus == 450 {
                    ATAppUpdater.shared().showForceUpdateWithoutVerifyVersion()
                }
            }
        ...
        if httpMethod == "PUT",
           httpPath == "/api/v1/message" {
            ...
            handleEmptyQueueMessage(message, currentWebSocket: currentWebSocket)
                  ...
                  httpPath == "/api/v1/conversation" {
            
            // If we receive a conversation preview message over the socket while the app is in the background,
            // prolong how long the socket stays open.
            //
            // TODO: NSE
            ensureBackgroundKeepAlive(.receiveMessage)
            
            handleIncomingConversationPreviewMessage(message, currentWebSocket: currentWebSocket)
        } else if httpPath == "/api/v1/queue/conversation/empty" {
            // Conversation preview message Queue is drained.
    ...
    private func handleIncomingMessage(_ message: WebSocketProtoWebSocketRequestMessage,
        ...
        let ackMessage = { (processingError: Error?, serverTimestamp: UInt64) in
            ...
            //            }
            MessageProcessor.handleMessageProcessingOutcome(error: processingError) { shouldAck, error in
                if shouldAck {
                    self.sendWebSocketMessageAcknowledgement(message, currentWebSocket: currentWebSocket)
                } else {
                    Logger.info("Skipping ack of message with serverTimestamp \(serverTimestamp) because of error: \(processingError?.localizedDescription ?? "nil!")")
                }
            }
        ...
        if let timestampString = headers.value(forHeader: "X-Signal-Timestamp") {
            if let timestamp = UInt64(timestampString) {
                ...
                Logger.debug("Invalidly formatted timestamp: \(timestampString)")
            }
        }
        
        if serverDeliveryTimestamp == 0 {
            Logger.debug("Missing server delivery timestamp")
        }
        ...
        guard let signalingKey = TSAccountManager.signalingKey() else {
            ackMessage(OWSGenericError("Missing signalingKey \(currentWebSocket.logPrefix)"), serverDeliveryTimestamp)
            return
        }
        ...
        Self.messageProcessingQueue.async {
            ...
            // TODO: msg processor swift
            Self.messageProcessor.processEncryptedEnvelopeData(decryptedPayload,
                                                               serverDeliveryTimestamp: serverDeliveryTimestamp,
                                                               envelopeSource: envelopeSource,
                                                               hotDataDestination: nil,
                                                               hotdataMsgFlag: false) { error in
                if Self.verboseLogging {
    ...
    private func handleIncomingConversationPreviewMessage(_ message: WebSocketProtoWebSocketRequestMessage,
                                                          currentWebSocket: WebSocketConnection) {
        assertOnQueue(Self.serialQueue)
        
        var backgroundTask: OWSBackgroundTask? = OWSBackgroundTask(label: "handleIncomingConversationPreviewMessage")
        
        if Self.verboseLogging {
            Logger.info("\(currentWebSocket.logPrefix) preview 1")
        }
        
        let ackMessage = { (processingError: Error?, serverTimestamp: UInt64) in
            if Self.verboseLogging {
                Logger.info("\(currentWebSocket.logPrefix) preview 2 \(processingError?.localizedDescription ?? "success!")")
            }
            
            ConversationPreviewProcessor.handleConversationProcessingOutcome(error: processingError) { shouldAck, error in
                if shouldAck {
                    self.sendWebSocketMessageAcknowledgement(message, currentWebSocket: currentWebSocket)
                } else {
                    Logger.info("Skipping ack of conversation preview message with serverTimestamp \(serverTimestamp) because of error: \(processingError?.localizedDescription ?? "nil!")")
                }
            }
        ...
        Self.messageProcessingQueue.async {
            if Self.verboseLogging {
                Logger.info("\(currentWebSocket.logPrefix) preview 2")
            }
            
            // TODO: msg processor swift
            Self.conversationPreviewProcessor.processConversationPreviewData(encryptedEnvelope,
                                                                             serverDeliveryTimestamp: serverDeliveryTimestamp) { error in
                if Self.verboseLogging {
                    Logger.info("\(currentWebSocket.logPrefix) preview 3")
                }
                Self.serialQueue.async {
                    ackMessage(error, serverDeliveryTimestamp)
                }
    ...
    private func handleEmptyQueueMessage(_ message: WebSocketProtoWebSocketRequestMessage,
        ...
        DTConversationPreviewManager.shared().needReportConversation = false;
...
//            return ""
//        case .identified:
//            let login = tsAccountManager.localNumber()?.replacingOccurrences(of: "+", with: "%2B") ?? ""
//            let password = tsAccountManager.serverAuthToken() ?? ""
...
//    }
    ...
    private func ensureWebsocketExists() {
        ...
        let mainServiceWebSocketAPI: String = {
            ...
            return DTServersConfig.convertToWebSocketUrl(withOriginUrl: TSConstants.mainServiceURL, serverType: .chat)
        }()
        
        Logger.info("\(self.logPrefix) webSocketAPI: \(mainServiceWebSocketAPI)")
        ...
        let httpHeaders = OWSHttpHeaders()
        httpHeaders.addDefaultHeaders()
        try? httpHeaders.addAuthHeader(username: OWSURLSession.loginHeaderValue, password: OWSURLSession.passwordHeaderValue)
...
extension OWSWebSocket: SSKWebSocketDelegate {
    ...
    public func websocketDidDisconnectOrFail(socket eventSocket: SSKWebSocket, error: Error?) {
        ...
        if let error = error {
            ...
            Logger.info("OWSWebsocket error start")
            if webSocketErrorStatusCode == 451,
               webSocketType == .identified {
                Logger.info("OWSWebsocket error 451 prepare showPopView")
               ActivePopViewManager.shared.showPopView()
            }
        }
...
private class WebSocketConnection {
    ...
    private var requestInfoMap = AtomicDictionary<UInt64, SocketRequestInfo>(lock: .sharedGlobal)
