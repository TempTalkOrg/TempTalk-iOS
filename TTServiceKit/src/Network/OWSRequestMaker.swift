...
public class RequestMaker: NSObject {
    ...
    public typealias RequestFactoryBlock = () -> TSRequest?
...
//        self.udAccess = udAccess
    ...
    public func makeRequest() -> Promise<RequestMakerResult> {
        return makeRequestInternal(skipWebsocket: false)
    }

    private func makeRequestInternal(skipWebsocket: Bool) -> Promise<RequestMakerResult> {
//        var udAccessForRequest: OWSUDAccess?
//        if !skipUD {
//            udAccessForRequest = udAccess
//        }
        let isUDRequest: Bool = false // udAccessForRequest != nil
        guard let request: TSRequest = requestFactoryBlock() else {
            return Promise(error: RequestMakerError.requestCreationFailed)
        }
        owsAssertDebug(isUDRequest == request.isUDRequest)
        let webSocketType: OWSWebSocketType = .identified // (isUDRequest ? .unidentified : .identified)
...
//                    self.profileManager.fetchProfile(for: self.address)
                    ...
                    if self.canFailoverUDAuth {
                        ...
                        return self.makeRequestInternal(skipWebsocket: skipWebsocket)
                    } else {
                ...
                if FeatureFlags.deprecateREST {
                    ...
                    return self.makeRequestInternal(skipWebsocket: true)
                }
            }
        } else {
            return firstly {
