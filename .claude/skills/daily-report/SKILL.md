---
name: daily-report
description: Generate a daily report from Git commits for today or a specified date
...
Generate a daily report based on Git commits for the specified date (or today if not specified).
...
3. ...
```
* 使用中文；每条一句话，描述本次提交/功能点的价值,每条以「<领域><动作><目的>」方式撰写。
* 列表项之间用两个空格结尾（保证 Markdown 换行）。

---
## 分析与整理

* **完整性检查**：仔细审查所有提交，确保不遗漏任何工作内容
* **包含范围 - 所有工作类型**：
  > 日报记录开发者的全部工作产出，包括技术性工作（agent 配置、工具链、CI 等）。
  - 功能实现 (feat)：新增功能、特性
...
* **反面示例 - 禁止这样合并**：
  ...
  应拆分为：
  ```
  ✅ Claude Code Agent 模型更新：优化 agent 配置。
  ✅ 本地分支清理技能：新增自动清理已合并分支功能。
  ✅ Figma 插件集成：启用 Figma MCP 插件。
  ```
* **大型 PR 拆分原则**：如果一个 PR 包含多个独立功能，应分别列出
* **输出原则**：列出所有对团队、产品、项目有意义的工作产出，无数量限制

## 中文总结格式

每条以「<领域><动作><目的>」方式撰写，示例：
* 消息撤回修复：解决最近聊天列表中撤回消息预览不正确的问题。
...
4. Claude Code 日报命令优化：支持包含所有工作类型，确保产出完整可见。
```
