...
public class ProfileFetcherJob: NSObject {
    ...
    public func run(recipientIds: [String]) {
        ...
        if (!(CurrentAppContext().isMainApp || CurrentAppContext().isNSE)) {
    ...
    public func updateProfile(recipientId: String, remainingRetries: Int = 3) {
        self.getProfile(recipientId: recipientId).done { profile in
    ...
    public func getProfile(recipientId: String) -> Promise<SignalServiceProfile> {
        ...
        }.map(on: DispatchQueue.main) { (result: RequestMakerResult) in
            guard let responseObject = result.responseJson as? [String: Any] else {
                throw OWSAssertionError("Profile fetch missing response object.")
            }
