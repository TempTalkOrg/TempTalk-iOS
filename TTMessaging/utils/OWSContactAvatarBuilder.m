...
// intern or reimplement this so that TTMessaging doesn't depend on JSQ
                 ...
                 contactsManager:(OWSContactsManager *)contactsManager
{
    ...
    if (name.length == 0) {
        [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction * transaction) {
            name = [contactsManager profileNameForRecipientId:signalId transaction:transaction];
        }];
    }
     */
    if (name.length == 0) {
                 ...
                 contactsManager:(OWSContactsManager *)contactsManager
{
    ...
    NSString *colorName = [TSThread stableConversationColorNameForString:signalId];
...
{
    return nil;
//    return [self.contactsManager profileImageForPhoneIdentifier:<#(nullable NSString *)#>:self.signalId];
...
- (UIImage *)buildDefaultImage
{
    ...
    [self.contactsManager.avatarCache imageForKey:[self imageKey] diameter:(CGFloat)self.diameter];
    if (cachedAvatar) {
        return cachedAvatar;
    }

    return [self buildDefaultImageForSave];
}

- (NSString *)imageKey {
    return [NSString stringWithFormat:@"%@_%@", self.signalId, self.contactName];
}
...
- (UIImage *)buildDefaultImageForSave
{
    ...
    if (rangeOfLetters.location != NSNotFound) {
        ...
        NSRange stringRange = { 0, MIN([initials length], (NSUInteger)1) }; // Rendering max 1 letters.
    ...
    [self.contactsManager.avatarCache setImage:image forKey:[self imageKey] diameter:self.diameter];
    return image;
}
...
- (UIImage *)buildImageForTemporary {
    ...
    if (self.contactName.length > 0) {
        initials = [self.contactName substringToIndex:1];
    }
