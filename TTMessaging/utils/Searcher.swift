...
public class Searcher<T> {
    ...
    public func matches(item: T, query: String, transaction: SDSAnyReadTransaction) -> Bool {
        ...
        return stem(string: query).compactMap { queryStem in
            return itemString.contains(queryStem)
        }.reduce(true) { $0 && $1 }
    }
    
    public func matchesEmailCharacters(item: T, query: String, transaction: SDSAnyReadTransaction) -> Bool {
        let itemString = normalize(string: indexer(item, transaction))
        return stemEmailCharacters(string: query).map { queryStem in
            return itemString.contains(queryStem)
        }.reduce(true) { $0 && $1 }
    }
    ...
    private func stemEmailCharacters(string: String) -> [String] {
        var normalized = normalize(string: string)
        return normalized.components(separatedBy: .whitespacesAndNewlines)
    }
