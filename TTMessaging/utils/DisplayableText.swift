...
@objc public class DisplayableText: NSObject {
    ...
    public class func displayableText(_ rawText: String) -> DisplayableText {
        ...
        //MARK: 设置文本消息折叠文本长度阀值
        let kMaxTextDisplayLength = kOversizeTextMessageSizeThreshold
        ...
        if displayText.count > kMaxTextDisplayLength {
            ...
            let snippet = String(displayText.prefix(Int(kMaxTextDisplayLength))).ows_stripped()
            displayText = String(format: Localized("OVERSIZE_TEXT_DISPLAY_FORMAT", comment:
