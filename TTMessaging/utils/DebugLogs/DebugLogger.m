...
+ (NSString *)agoraLogsDirPath
{
    NSString *dirPath = [OWSFileSystem cachesDirectoryPath];
    [OWSFileSystem ensureDirectoryExists:dirPath];
    return dirPath;
}
...
- (void)wipeLogs
{
    ...
    if (reenableLogging) {
        [self enableFileLoggingWithAppContext:CurrentAppContext() canLaunchInBackground:YES];
    }
}
