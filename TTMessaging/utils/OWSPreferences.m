...
- (nullable id)tryGetValueForKey:(NSString *)key transaction:(SDSAnyReadTransaction *)transaction
{
    OWSAssertDebug(key != nil);
    return [self.keyValueStore getObjectForKey:key transaction:transaction];
}
...
- (void)setValueForKey:(NSString *)key toValue:(nullable id)value
{
    ...
    DatabaseStorageAsyncWrite(self.databaseStorage, ^(SDSAnyWriteTransaction *writeTransaction) {
        [self setValueForKey:key toValue:value transaction:writeTransaction];
    });
}
           ...
           transaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    [self.keyValueStore setObject:value key:key transaction:transaction];
}
...
// recent calls list.
- (void)applyCallLoggingSettingsForLegacyUsersWithTransaction:(SDSAnyWriteTransaction *)transaction
{
    ...
    // We need to reload the callService.callUIAdapter here, but TTMessaging doesn't know about CallService, so we use
    // notifications to decouple the code. This is admittedly awkward, but it only happens once, and the alternative would
    // be importing all the call related classes into TTMessaging.
...
- (NSString *)nameForNotificationPreviewType:(NotificationType)notificationType
{
    switch (notificationType) {
        case NotificationNamePreview:
            return Localized(@"NOTIFICATIONS_SENDER_AND_MESSAGE", @"");
        case NotificationNameNoPreview:
            return Localized(@"NOTIFICATIONS_SENDER_ONLY", @"");
        case NotificationNoNameNoPreview:
            return Localized(@"NOTIFICATIONS_NONE", @"");
        default:
...
- (void)endCallKitCallWithRoomId:(NSString *)roomId {
    ...
    CFStringRef name = CFSTR("com.temptalk.nseCallkitStop");
