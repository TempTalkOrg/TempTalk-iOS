...
public class OWSNavigationBar: UINavigationBar {
    ...
    private var _appearanceBlurEffectView: UIVisualEffectView?

    private var appearanceBlurEffectView: UIVisualEffectView? {
        get { _appearanceBlurEffectView }
        set {
            if _appearanceBlurEffectView !== newValue {
                // 旧值清理
            ...
            _appearanceBlurEffectView = newValue
        }
    }
    ...
    fileprivate func updateAppearanceBlurView() {
        guard let blurView = appearanceBlurEffectView else {
            appearanceBlurEffectView = nil
            return
        }

        guard let tintingView = blurView.tintingView else {
            appearanceBlurEffectViewBackgroundColorObservation?.invalidate()
            appearanceBlurEffectViewBackgroundColorObservation = nil

            if appearanceBlurEffectViewSublayerObservation == nil {
                // blurView 初始时没有 subviews，用 KVO 等待 sublayers 创建
                appearanceBlurEffectViewSublayerObservation?.invalidate()
                appearanceBlurEffectViewSublayerObservation = blurView.layer.observe(\.sublayers) { [weak self] _, _ in
                    self?.updateAppearanceBlurView()
                }
            }
            return
        ...
        blurView.matchBackgroundColor(desiredBgColor)
