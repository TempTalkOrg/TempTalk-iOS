...
@property (nonatomic) NSArray<SignalAccount *> *bots;
...
- (BOOL)isCurrentUser:(SignalAccount *)signalAccount
{
    ...
    for (NSString *phoneNumber in signalAccount.contact.userTextPhoneNumbers) {
        if ([phoneNumber isEqualToString:localNumber]) {
            return YES;
        }
    }
...
- (void)updateContacts
{
    ...
    self.bots = self.contactsManager.bots;
                                                     ...
                                                     transaction:(SDSAnyReadTransaction *)transaction;
{
    ...
    return [searcher searchSignalAccountsWithSearchWord:searchText
                                            transaction:transaction];
//    return [self.fullTextSearcher filterSignalAccounts:self.signalAccounts withSearchText:searchText transaction:transaction];
}
...
+ (void)presentMissingContactAccessAlertControllerFromViewController:(UIViewController *)viewController
{
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:Localized(@"EDIT_CONTACT_WITHOUT_CONTACTS_PERMISSION_ALERT_TITLE",
                                           @"Alert title for when the user has just tried to edit a "
                                                     @"contacts after declining to give Signal contacts "
                                                     @"permissions")
                         message:Localized(@"EDIT_CONTACT_WITHOUT_CONTACTS_PERMISSION_ALERT_BODY",
    ...
    [alertController
        addAction:[UIAlertAction actionWithTitle:Localized(@"AB_PERMISSION_MISSING_ACTION_NOT_NOW",
...
//    }
    ...
    if (!contactViewController) {
        ...
        [self.databaseStorage readWithBlock:^(SDSAnyReadTransaction * transaction) {
            newContact.givenName = [self.profileManager profileNameForRecipientId:recipientId transaction:transaction];
        }];
