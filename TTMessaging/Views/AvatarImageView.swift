...
public class ConversationAvatarImageView: AvatarImageView {
    ...
    required public init(thread: TSThread, diameter: UInt, contactsManager: OWSContactsManager) {
        ...
        default:
            owsFailDebug("in \(#function) unexpected thread type: \(thread)")
            self.recipientId = nil
            self.groupThreadId = nil
        }
    ...
    @objc func handleGroupAvatarChanged(notification: Notification) {
        ...
        self.databaseStorage.read { [self] transaction in
            let latestThread = TSThread.anyFetch(uniqueId: self.thread.uniqueId, transaction: transaction);
            if let value = latestThread?.dictionaryValue {
                thread.setValuesForKeys(value)
            }
        }
