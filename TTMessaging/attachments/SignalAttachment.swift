...
extension SignalAttachmentError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .couldNotParseImage:
            return Localized("ATTACHMENT_ERROR_COULD_NOT_PARSE_IMAGE", comment: "Attachment error message for image attachments which cannot be parsed")
        case .couldNotConvertToJpeg:
            return Localized("ATTACHMENT_ERROR_COULD_NOT_CONVERT_TO_JPEG", comment: "Attachment error message for image attachments which could not be converted to JPEG")
        case .invalidFileFormat:
            return Localized("ATTACHMENT_ERROR_INVALID_FILE_FORMAT", comment: "Attachment error message for attachments with an invalid file format")
        case .couldNotConvertToMpeg4:
            return Localized("ATTACHMENT_ERROR_COULD_NOT_CONVERT_TO_MP4", comment: "Attachment error message for video attachments which could not be converted to MP4")
        case .couldNotRemoveMetadata:
            return Localized("ATTACHMENT_ERROR_COULD_NOT_REMOVE_METADATA", comment: "Attachment error message for image attachments in which metadata could not be removed")
        case .couldNotResizeImage:
            return Localized("ATTACHMENT_ERROR_COULD_NOT_RESIZE_IMAGE", comment: "Attachment error message for image attachments which could not be resized")
        }
    }
}
...
//        return MIMETypeUtil.supportedImageUTITypes().union(animatedImageUTISet)
    ...
    private class func imageAttachment(dataSource: DataSource?, dataUTI: String, imageQuality: TSImageQuality) -> SignalAttachment {
        ...
        if animatedImageUTISet.contains(dataUTI) {
            ...
            if isValidOutput {
                ...
                return compressImageAsJPEG(dataSource: dataSource, attachment: attachment, filename: dataSource.sourceFilename, imageQuality: imageQuality)
            }
        }
    }
    ...
    public class func imageAttachment(image: UIImage?, dataUTI: String, filename: String?, imageQuality: TSImageQuality) -> SignalAttachment {
        ...
        guard let imageData = image?.jpegData(compressionQuality: 1),
              let dataSource = DataSourceValue.dataSource(with: imageData, utiType: dataUTI)else {
        ...
        return compressImageAsJPEG(dataSource: dataSource, attachment: attachment, filename: filename, imageQuality: imageQuality)
    }

    private class func compressImageAsJPEG(dataSource: DataSource, attachment: SignalAttachment, filename: String?, imageQuality: TSImageQuality) -> SignalAttachment {
        ...
        while true {
            ...
            if image.size.width > maxSize || image.size.height > maxSize {
                guard let resizedImage = downsampleImage(dataSource: dataSource, toMaxSize: maxSize) else {
    ...
    private class func downsampleImage(dataSource: DataSource, toMaxSize maxSize: CGFloat) -> UIImage? {
        autoreleasepool {
    ...
    private class func cgImageSource(for dataSource: DataSource) -> CGImageSource? {
        if let dataUrl = dataSource.dataUrl() {
    ...
    public static func compressVideoAsMp4(
        ...
        baseFilename: String?,
        dataUTI: String,
        presentName: String = AVAssetExportPreset640x480,
        sessionCallback: ((AVAssetExportSession) -> Void)? = nil
    ) async throws -> SignalAttachment {
        Logger.debug("")
        guard let exportSession = AVAssetExportSession(asset: asset, presetName: presentName) else {
            let attachment = SignalAttachment(dataSource: DataSourceValue.emptyDataSource(), dataUTI: dataUTI)
            attachment.error = .couldNotConvertToMpeg4
            return attachment
        }
    ...
    public class func attachment(dataSource: DataSource?, dataUTI: String) -> SignalAttachment {
        if inputImageUTISet.contains(dataUTI) {
            Logger.warn("must specify image quality type")
        }
